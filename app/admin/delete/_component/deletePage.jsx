...
export default function DeletePage(){
    ...
    async function formAction(formData){ //This function is used to manage the response of the server action
        const response = await deletePost(formData); //call to the server action
    ...
    return(
        <main className="flex flex-col items-center">
            ...
            <form action={formAction} className="flex flex-col text-lg w-[80vw] md:w-[50vw] lg:w-[50vw] xl:w-[30vw] border card-bg px-8">
                ...
                <label className="self-center m-4" htmlFor="id">ID</label>
                <input className="py-2 px-3 border focus:outline-none focus:outline-red-800 focus:outline-offset-0 input-bg rounded border-amber-400" type="text" placeholder="3f123jh12g3jhgy" name="id" />
                <div className="flex my-10 flex-col lg:justify-center lg:flex-row">
                    <button className="mb-6 lg:mb-0 border-2 border-red-800 bg-amber-400 font-extrabold text-nowrap rounded-xl max-w-fit self-center py-1 px-6 text-red-800" type="submit">Delete</button> 
                </div>
            </form>
        </main>
    )
